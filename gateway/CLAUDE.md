...
│   ├── llm_proxy.py     — /llm/v1 proxy for user machines → OpenRouter
...
│   ├── llm.py           — LLM proxy logic (budget tracking, model allowlists, OpenRouter forwarding)
│   └── supabase_client.py — Supabase service (user machines CRUD, subscriptions, chat, Slack connections)
├── middleware/
