...
function setPluginModuleLoaderJitiFactoryOverride(factory: unknown): void {
  (
    globalThis as typeof globalThis & {
      [pluginModuleLoaderJitiFactoryOverrideKey]?: unknown;
    }
  )[pluginModuleLoaderJitiFactoryOverrideKey] = factory;
}
...
describe("bundled plugin public surface loader", () => {
  ...
  it("prefers source require for bundled source public artifacts when a ts require hook exists", async () => {
    const typeScriptFeatureSpy = vi
      .spyOn(process.features, "typescript", "get")
    ...
    expect(createJiti).not.toHaveBeenCalled();
    typeScriptFeatureSpy.mockRestore();
  });

  it("uses the source transformer instead of Node's native TypeScript require hook", async () => {
    const typeScriptFeatureSpy = vi
      .spyOn(process.features, "typescript", "get")
      .mockReturnValue("strip");
    const transformedLoader = vi.fn(() => ({ marker: "source-transformer-ok" }));
    const createJiti = vi.fn(() => transformedLoader);
    setPluginModuleLoaderJitiFactoryOverride(createJiti);
    const requireLoader = Object.assign(
      vi.fn(() => ({ marker: "native-strip-should-not-run" })),
      {
        extensions: {
          ".ts": vi.fn(),
        },
    ...
    const publicSurfaceLoader = await importFreshModule<
      typeof import("./public-surface-loader.js")
    >(import.meta.url, "./public-surface-loader.js?scope=native-typescript-source-transformer");
    const tempRoot = createTempDir();
    const bundledPluginsDir = path.join(tempRoot, "extensions");
    process.env.OPENCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;

    const modulePath = path.join(bundledPluginsDir, "demo", "provider-policy-api.ts");
    fs.mkdirSync(path.dirname(modulePath), { recursive: true });
    fs.writeFileSync(modulePath, 'export const marker = "source-transformer-ok";\n', "utf8");
    ...
    ).toBe("source-transformer-ok");
    expect(transformedLoader).toHaveBeenCalledWith(fs.realpathSync(modulePath));
