...
// Gateway mode auto-connect: relay connects on startup, agent creates its own tabs.
let gatewayAutoConnect = false
...
async function connectOrToggleForActiveTab() {
  ...
  try {
    ...
    void chrome.action.setTitle({
      tabId,
      title: 'Magister Browser Control: connecting to relay…',
    })
...
async function showClickIndicator(tabId, x, y) {
  const script = `(() => {
    ...
    el.style.cssText = 'position:fixed;left:${x - 12}px;top:${y - 12}px;width:24px;height:24px;border-radius:50%;background:rgba(255,59,48,0.4);border:2px solid rgba(255,59,48,0.8);pointer-events:none;z-index:2147483647;transition:opacity 0.6s ease-out,transform 0.6s ease-out;transform:scale(1)';
  ...
  try {
    ...
    // Page may not allow script injection.
  }
}
...
async function handleForwardCdpCommand(msg) {
  ...
  // Target.createTarget doesn't need an existing tab — handle before tab lookup.
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (tabs.size === 0 && !gatewayAutoConnect) return
...
initPromise.then(async () => {
  // In gateway mode, always connect the relay on startup so auto-attach works.
  const mode = await getConnectionMode()
  const shouldConnect = tabs.size > 0 || mode.mode === 'gateway'
  if (shouldConnect) {
